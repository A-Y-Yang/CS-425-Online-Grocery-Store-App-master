...
@app.route('/addstocks/<int:id>', methods=['GET', 'POST'])
def addstocks(id):
    if 'email' not in session:
        flash(f'Please login first','danger')
        return redirect(url_for('home'))
    ...
    if request.method == 'POST' and form.validate():
        stock = Stock(warehouse_id = id, product_id = request.form.get('product_id'),
                  item_quantity =  form.item_quantity.data, size_total = form.size_total.data )
        db.session.add(stock)
        db.session.commit()
        flash(f'Stock is added to this warehouse.', 'success')
    return render_template('admin/addstock.html', title = "Add Stick Page", form = form, products = products, warehouse = warehouse)